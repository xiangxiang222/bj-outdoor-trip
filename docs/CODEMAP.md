...
| `virtual.js` | 虚拟用户池（上限 2000）、按团抽人占座，真人报名腾座 |
| `virtual-heat.js` | 自动热度：规则 A 水位、夜间静默、出发前清空；5 分钟任务在 `index.js` |
...
| `/m/wallet/cards` | WalletCards | 提示已改为微信零钱，回钱包 |
| `/m/wallet/pin` | WalletPin | 6 位支付密码；可用身份证重置 |
| `/m/settings` | Settings | 实名、支付密码、退出/注销 |
