...
| `wallet.js` | 余额账本、充值/提现、银行卡掩码、6 位支付密码；返点与成团奖励入账 |
...
| `/m/mine` | Mine | 资料、会员条、行程快捷入口、钱包余额条、常用宫格 |
| `/m/wallet` | Wallet | 余额、充值、提现、账单 |
| `/m/wallet/cards` | WalletCards | 绑定银行卡（只存开户行和后四位） |
| `/m/wallet/pin` | WalletPin | 6 位支付密码；可用身份证重置 |
| `/m/settings` | Settings | 实名、支付密码、银行卡、退出/注销 |
| `/m/profile` | Profile | 实名身份证（展示脱敏） |
...
- 「我的」顶部有设置和钱包余额条（充值/提现/账单、银行卡只留后四位、支付密码）；常用宫格有收藏、认证、发团、抽奖、收录线路。没有团体/推荐报名入口（这些在 H5 有）。点团详情「报名领队」未认证时弹窗跳到领队申请。点「报名摄影师」未报名时进报名页。
...
我的 GET /me + GET /me/wallet + GET /me/coupons
