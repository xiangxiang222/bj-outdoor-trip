...
| Tab | 内容 |
| --- | --- |
| 首页 | 山野发现。状态栏图形标 + 活字「同行者众」+ 口号；圆角景点轮播（可点进线路）；薄学生认证条；城市/玩法胶囊；横滑日期；feed 活动卡；发团与筛选 |
...
- **首页结构**：圆角可点轮播 → 薄学生认证条 → 热门/城市胶囊 → 玩法胶囊 → 十五天横滑日期 → 「看看最近都在忙什么」+ 筛选/发团 → 左图右文 feed 卡（时间、标题、主理人、玩法条、已上车人数）。月/节日/特价收到「筛选」里。
- **活动 Tab**：同城局同样用 feed 卡；分类宫格选中为荧光绿。
