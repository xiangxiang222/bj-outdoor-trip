...
非目标（后续迭代）：真实短信网关、微信原路退款。候补、座位图、保险加购、天气提醒、企业支付分账、导游 H5 工作台、出行评价已落地。
...
| `server/src/services/` | 身份证、报价、画像、导游匹配、微信 mock、验证码、解散、取消报名、注销、候补座位保险天气分账评价 |
...
用户端（`/m`）：首页发现（天数入口、目的地、正在拼团、周末短途）→ 线路目录筛选/详情（含评价）→ 排期详情与报名名单 → 报名占座 / 开团 → 我的（订单、取消、评价、收藏、开通会员、注销）。导游端（`/g`）：手机号登录后看出行名单并签到。
