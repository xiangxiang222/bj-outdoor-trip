...
高校赞助等车位有限的团可开 `oversub`。对外文案是「报超会抽」：报名人数超过座位才抽签，未超过则全部确认，不说「抽名额」。这与行前/行后积分抽奖（`lottery_draws`）分开。
...
| Tab | 内容 |
| --- | --- |
| 首页 | 山野发现。状态栏图形标 + 活字「同行者众」+ 口号；圆角景点轮播（可点进线路）；薄学生认证条；搜索；城市/玩法胶囊；feed 活动卡（含满员可候补）；排序/筛选/发团 |
...
- **首页结构**：圆角可点轮播 → 薄学生认证条 → 搜索 → 热门/城市胶囊 → 玩法胶囊 → 「看看最近都在忙什么」+ 即将出发/筛选/发团 → 左图右文 feed 卡（时间、标题、主理人、玩法条、已上车人数；满员显示「已满·可候补」）。十五天日历、月/节日/特价收到「筛选」里，不挡发现。
