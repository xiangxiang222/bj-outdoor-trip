...
非目标（后续迭代）：真实短信网关、候补排队、座位图、保险加购、企业微信支付分账、导游端小程序。取消/解散已做演示态退款标记，未对接微信原路退款。
...
| `server/src/services/` | 身份证、报价、画像、导游匹配、微信 mock、验证码、解散、取消报名、注销 |
...
用户端（`/m`）：首页发现（天数入口、目的地、正在拼团、周末短途）→ 线路目录筛选/详情 → 排期详情与报名名单 → 报名占座 / 开团 → 我的（订单与取消、收藏、开通会员、注销）。
