...
| `WX_APPID` / `WX_APPSECRET` / `WX_MCH_ID` / `WX_MCH_KEY` | 小程序与商户 | 默认 AppID `wx255ca387929c502a`、商户号 `1750196084`；Secret / APIv2 密钥只放服务器 `.env` |
...
- `status`：`on` 上架 / `off` 下架（删除接口实际为下架）；用户申请收录为 `pending` 直到后台通过
...
- 虚拟用户池上限 2000。官方团和用户自建团默认开自动热度（规则 A：展示钉在成团人数约 45% 且低于成团线；真人 +1 则虚拟 −1）。加密团、同城局默认关。出发前 24 小时清掉虚拟占座。铁定出发 / 成团奖 / 导游匹配 / 并团只看真人。前台不暴露虚拟标记。后台可手改人数（会暂停自动）或用虚拟用户写评价。

详情页若已有 `myEnrollment`，H5 / 小程序不再露出报名按钮。

### 3.3.2 报超会抽与校园认证

高校赞助等车位有限的团可开 `oversub`。对外文案是「报超会抽」：报名人数超过座位才抽签，未超过则全部确认，不说「抽名额」。**高校免费团**（`organizer_type=campus` 且 `offer_type=free`）发布时默认打开抽签，并用学校名写入限定高校。这与行前/行后积分抽奖（`lottery_draws`）分开。发团可选 `campusScope`：仅本学院、仅本校、本校跨学院、跨学校。跨学校写入 `campus_targets_json`（多条学校-学院-专业组合，三项绑定，学院/专业可空）；两校同名学院要分成两组，不能把学院名并成一份名单。发起人之后只能 `PUT /schedules/:id/limit` 再开放组合，不能收窄。
...
- `users.campus_kind`：`student` 在读师生 / `alumni` 校友。另有 `college` `major` `student_no` `student_card_url`。校友通过认证后 `is_student=0`，不享受学生价。排期 `alumni_ok` 打开后，指定高校的已认证校友可报。排期 `campus_targets_json` 为 `[{school,college,major}]`；`schools_json`/`colleges_json` 是派生快照。报名命中任一条组合即可（有学院则学院也要对上，有专业则专业也要对上）。无 `campus_targets_json` 的旧数据按 `schools`×`colleges` 笛卡尔积兼容
...
- 学生：`POST /me/student` 可选学校、学院、专业（北京名录 `GET /campuses` 搜索分页，学校→学院→专业一对多级联），在读须学号并上传学生证 → `student_status=pending` → 后台 `POST /admin/users/:id/verify` `kind=student` 通过后 `is_student=1`。部分团 `studentOnly` 或 `campusTargets` 组合（名称包含匹配）。用户认证仍是一组学校-学院-专业；发团可配多组，按整组匹配，不把多校学院名混在一起。提交时写入 `admin_notices`，后台消息点开 `/admin/verify?kind=campus&userId=`。学号和证件只给本人与后台。
...
- 钱包：`users.wallet_balance` 整数元。分享报名返点 5%、线路首次成团 300、个人发团成团 200 入账。充值走微信支付，提现到微信零钱（商家转账，商户号能力，不需要支付牌照）。不支持绑定银行卡进出金。余额可付团费。流水在 `wallet_ledger`。提现须实名 + 6 位支付密码。不设最低提现金额、不限制每日次数；微信转账单笔上限 2000 元，超出可立即分笔。全天可提，提交成功后实时到账、最迟 24 小时。`bank_cards` 表保留但接口已关闭
...
2. **官方团**：后台可手开；上架线路会滚动补近 10 日出发展。出行前一天，同一线路、同一集合时间与地点还没成团的其他团并入官方团；人数超过座位则换更大车。并入后给出行人发短信，有微信 openid 的再发小程序订阅消息。
3. **同城局**：活动 Tab「发起一局」→ `POST /trips` 且 `channel=activity`，选掼蛋/跑步/电影/招募、地点、时间、人数，不必选大巴。审核通过后出现在活动 Tab。
4. 分享：小程序原生转发卡片（标题含价格，封面为生成的报名图，path 带推荐码 `ref`）；H5 调海报接口拿到详情图+二维码（SVG 把封面内嵌成 data URL，当 `<img>` 用时才能显示）；短链 `/api/share/:token`（302 到排期页，可带 `ref`）。登录后分享绑定推荐人，好友报名成团返点 5%。
