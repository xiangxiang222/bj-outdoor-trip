...
- 钱包：`users.wallet_balance` 整数元。分享报名返点 5%、线路首次成团 300、个人发团成团 200 入账。充值走微信支付，提现到微信零钱（商家转账，商户号能力，不需要支付牌照）。不支持绑定银行卡进出金。余额可付团费。流水在 `wallet_ledger`。提现须实名 + 6 位支付密码。`bank_cards` 表保留但接口已关闭
