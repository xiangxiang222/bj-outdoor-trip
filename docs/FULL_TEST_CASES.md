...
对照日期 2026-09-13。本文把 **注册、高校 / 公司 / 个人、领队、摄影师、发团、抽签、优惠券、抽奖、管理员、用户端、导游端** 的用例放到同一条测试顺序里。
...
2. **交互规则表（§2）**：每一条「允许 / 禁止」的组合至少对应一条自动化用例。
3. **手动走查（§3）**：按真实使用顺序走三端。每个模块、每种用户身份、每种团型至少出现一次。
...
下表每一行都是产品约束。UI 列指向 §3 的步骤；自动化列指向 `server/test/`。
...
公司团发券 / 报名带券的失败路径由 `api.coupon.test.js` 覆盖，后台对话框已写「公司团不可用」。
...
`miniprogram/` 打同一套 API。本走查在 H5；小程序用微信开发者工具按相同顺序点即可。
...
用户端入口见 M-69。官方「新增线路 → 发布拼团」与用户发团待审见 `docs/PUBLISH_TEST_CASES.md`。
