...
本文把「发布行程」从入口到上架的用例收成一份：自动化对照和手工步骤。
...
小程序对应页：`pages/publish/publish`、`pages/open/open`、`pages/route-apply/route-apply`。用户端在 H5 演示壳里走，后台用桌面宽度。微信开发者工具需另行打开真机预览。
...
期望列写的是产品行为。本次后台走查新增线路标题为「百花山一日徒步（后台发布）」。
