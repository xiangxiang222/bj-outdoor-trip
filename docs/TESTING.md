...
| `api.eligibility.test.js` | 仅学生 / 高校名单 / 绑定的学校-学院-专业组合（两校同名学院不混） / 高校免费团仍检查学校限制；发起人只扩不缩 |
| `api.campuses.test.js` | 北京高校名录搜索分页；学院按学校、专业按学院级联；认证学校/学院/专业可空，证件仍必填 |
...
| `api.coupon.test.js` | 公开限量领取、每人一张、会员与券取低不叠、赠团不核销、候补占用/递补核销/取消退券、公司团与暂停领取、仅会员领取、定向发放与演示短信、领取后过期、通用券、叠加会员、久未参加/出行次数、人多随机发放、免费券、用户搜索分页选人、公开/会员指定必领与预留库存、按高校名单筛选并发放 |
...
| `api.route-apply.test.js` | 用户申请收录线路、待审对路人隐藏、后台审批、首次成团奖励 300 元只发一次（须实付后）、驳回不发奖 |
| `api.official-trip.test.js` | 后台官方团、用户不能发官方团、按日补齐、个人成团奖 200（须实付后）、出行前一天并入并扩座，并短信+小程序通知 |
...
node scripts/e2e.js --live --base https://togetherbetter.cn
```
