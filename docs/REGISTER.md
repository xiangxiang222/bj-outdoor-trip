...
现有服务器 IP：140.143.171.77（腾讯云轻量，已有账号可继续用）
...
3. 支付回调目前是 `http://140.143.171.77/api/pay/wechat/notify`。正式环境微信要求 **HTTPS + 已备案域名**，并在小程序后台配 request / upload 合法域名。没有 HTTPS 时，小程序付款后会走服务端查单确认。
