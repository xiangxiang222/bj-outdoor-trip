...
const rows = [
  ...
  { id: 5, status: "recruiting", reviewStatus: "approved", channel: "trip", city: "怀柔", startDate: "2026-09-21", meetupTime: "07:30", enrolled: 4, maxSeats: 30, remain: 26, route: { title: "慕田峪官方" }, organizerName: "同行者众", organizerType: "official", companyName: "同行者众" },
...
describe("processFeed", () => {
  it("keeps full trips for waitlist instead of dropping remain=0", () => {
    ...
    expect(list.map((r) => r.id)).toEqual([1, 2, 3, 5, 4]);
  });
  ...
  it("searches title, city and host", () => {
    ...
    expect(processFeed(rows, { query: "怀柔", sort: "soon" }).map((r) => r.id)).toEqual([2, 3, 5]);
  ...
  it("filters company, campus and a named school", () => {
    ...
    expect(processFeed(rows, { hostKind: "official" }).map((r) => r.id)).toEqual([5]);
