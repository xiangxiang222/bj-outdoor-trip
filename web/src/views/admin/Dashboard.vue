<template>
  <div>
    ...
    <p v-if="d.pending" class="admin-alert">公司挂账待结算 {{ d.pending }} 人，请到「拼团与成本」结算。</p>
    ...
    <div class="card admin-dash-table">
      <el-table :data="d.byRoute" stripe>
...
onMounted(async () => {
  ...
  echarts.init(bar.value).setOption({
    ...
    series: [
      ...
      { name: "收入", type: "line", yAxisIndex: 1, data: d.byRoute.map((x) => x.revenue), itemStyle: { color: "#ff8f1f" } },
    ],
  });
