...
function dockPrice(row) {
  const q = (row && row.quote) || {};
  const origin = Number(q.originPrice != null ? q.originPrice : q.tripPrice != null ? q.tripPrice : q.price || 0);
  const trip = Number(q.tripPrice != null ? q.tripPrice : q.price != null ? q.price : origin);
  const member = Number(q.memberPrice != null ? q.memberPrice : trip);
  if (origin === 0 && trip === 0) return { free: true, main: "免费", sub: "" };
  if (row && row.channel === "activity") return { free: false, main: "¥" + trip, sub: "" };
...
function ticketState(row, query) {
  query = query || {};
  if (!row) return null;
...
module.exports = { timeLine, peopleLine, trustChips, dockPrice, enrollCta, canShowEnroll, ticketState };
